...
【当前状态】
- 当前使用 RTX 4090 (24GB) 实例，约2元/小时，运行Qwen2.5-7B-Instruct
- 完整实现了开机→SSH部署→启动服务→空闲关机全流程
- 支持余额不足/GPU繁忙等异常状态的检测和提示
- asyncssh为可选依赖，未安装时SSH功能不可用

【机型升级方案】
AutoDL提供多种GPU机型，升级后可解锁更大模型和更快推理：
- RTX 4090 (24GB, ~2元/h)：当前方案，跑7B模型流畅，14B勉强
- A100 (40GB, ~5元/h)：可跑14B-32B模型，推理速度更快
- A100 (80GB, ~10元/h)：可跑70B模型（如Qwen2.5-72B），回答质量显著提升
- 多卡A100 (2x80GB, ~20元/h)：可跑70B+长上下文（32K），支持更复杂推理

【未来改进方向】
- 增加模型热切换（开机后通过API切换不同模型，无需重启）
- 支持按需选择机型（简单问题用7B，复杂问题自动升级到32B/70B）
- 加入推理性能监控（首字延迟、吞吐量），自动调整并发数
- 实现多用户排队和会话隔离，支持多人同时测试
- 增加费用预算控制（日预算/月预算上限，超限自动关机）
"""
import asyncio
import json
import os
